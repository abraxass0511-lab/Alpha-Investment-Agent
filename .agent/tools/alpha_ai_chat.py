...
def gather_context():
    ...
    try:
        import pandas as pd
        mom_path = "output_reports/sentiment_all_latest.csv"
        if os.path.exists(mom_path):
            df = pd.read_csv(mom_path)
            context["momentum_data"] = df.to_dict(orient="records")
        else:
            context["momentum_data"] = []
    except:
        context["momentum_data"] = []
...
## 투자 전략 (5단계 필터)
1+2단계: 체급+내실 — 시총 $10B 이상, ROE 15% 이상
3단계: 에너지 — 주가 > 50일 이동평균선
4단계: 성장 — 어닝 서프라이즈 10% 이상
5단계: 모멘텀 Elite 5 — 12개월-1개월 모멘텀 상위 5종목 선정

## 매도 기준
- 트레일링 스톱: 고점 대비 -10% 하락 시 자동 매도
- 리밸런싱: 1~4단계 중 하나라도 탈락하면 매도 추천
...
def build_prompt(user_question, context):
    ...
    if context.get("scan_metadata"):
        ...
        ctx_text += f"### 스캔 결과\n"
        ctx_text += f"- 1+2단계(체급+내실): {m.get('step12', 0)}건 통과\n"
        ctx_text += f"- 3단계(에너지): {m.get('step3', 0)}건 통과\n"
        ctx_text += f"- 4단계(성장): {m.get('step4', 0)}건 통과\n"
        ctx_text += f"- 5단계(모멘텀 Elite 5): {m.get('step5', 0)}건 통과\n\n"

    if context.get("final_picks"):
        ctx_text += f"### 최종 선정 종목\n"
        for p in context["final_picks"]:
            mom = p.get('Momentum_12_1', 'N/A')
            ctx_text += f"- {p.get('Symbol', '?')}: 모멘텀 {mom}, 근거: {p.get('Reason', 'N/A')}\n"
        ctx_text += "\n"
    ...
    if context.get("momentum_data"):
        ctx_text += f"### 모멘텀 데이터 (상위 10개)\n"
        sorted_mom = sorted(context["momentum_data"], key=lambda x: x.get("Momentum_12_1", 0), reverse=True)[:10]
        for s in sorted_mom:
            mom_val = s.get('Momentum_12_1', 0)
            ctx_text += f"- {s.get('Symbol', '?')}: 12-1 모멘텀 {mom_val}\n"
        ctx_text += "\n"
